...
Using Official SIH Template Background (Clean White + SIH Brain Bulb + Hexagons)
...
# ── Colors ──
DARK_NAVY = RGBColor(0x1B, 0x36, 0x5D)      # Official SIH Header Blue
SIH_ORANGE = RGBColor(0xE6, 0x51, 0x00)     # Official SIH Orange
FOREST_GREEN = RGBColor(0x00, 0x8A, 0x4B)   # Clean Green Accent
TEXT_DARK = RGBColor(0x1E, 0x29, 0x3B)      # Primary Charcoal Text
TEXT_MUTED = RGBColor(0x64, 0x74, 0x8B)     # Slate Grey Subtext
CARD_BG = RGBColor(0xFA, 0xFA, 0xFC)        # Crisp Off-White Card Fill
CARD_BORDER = RGBColor(0xCB, 0xD5, 0xE1)    # Light Border
...
def add_header_footer(slide, slide_num, title_text=None):
    ...
    top_left = slide.shapes.add_textbox(Inches(0.8), Inches(0.25), Inches(4.5), Inches(0.35))
    ...
    # Top Header Right: SIH branding area
    top_right = slide.shapes.add_textbox(Inches(6.8), Inches(0.25), Inches(5.733), Inches(0.35))
    ...
    # Footer Left
    fl = slide.shapes.add_textbox(Inches(0.8), Inches(7.05), Inches(9.5), Inches(0.3))
    tf_fl = fl.text_frame
    ...
    # Footer Right
    fr = slide.shapes.add_textbox(Inches(10.5), Inches(7.05), Inches(2.033), Inches(0.3))
    tf_fr = fr.text_frame
...
# ============================================================
# SLIDE 1: TITLE SLIDE (Matches Template Background Exactly)
# ============================================================
slide1 = prs.slides.add_slide(blank_layout)
# Set background template image
slide1.shapes.add_picture("sih_slide1_bg.png", 0, 0, SLIDE_W, SLIDE_H)

# Main Title & Subtitle in Serif/Modern Blue
tbox = slide1.shapes.add_textbox(Inches(0.8), Inches(0.75), Inches(8.8), Inches(1.3))
ttf = tbox.text_frame
ttf.word_wrap = True
p = ttf.paragraphs[0]
...
p_m.text = "AI-Based Early Warning & Landslide Monitoring System"
p_m.font.bold = True
p_m.font.size = Pt(18)
...
# Bullet Points on the Left Side (Matching SIH Official Template Style)
meta_card = slide1.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(2.15), Inches(7.6), Inches(2.35))
...
for i, (k, v) in enumerate(bullets):
    ...
    r = p.add_run()
    r.text = v
    r.font.bold = (k in ["Team Name", "Team Leader", "Problem Statement ID"])
    r.font.size = Pt(10)
    r.font.color.rgb = SIH_ORANGE if k in ["Team Name", "Team Leader"] else TEXT_DARK

# Clean Team Table Below Bullets (All 6 Members with Roles & Responsibilities)
team_box = slide1.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(4.6), Inches(7.6), Inches(2.3))
team_box.fill.solid()
team_box.fill.fore_color.rgb = CARD_BG
team_box.line.color.rgb = CARD_BORDER
team_box.line.width = Pt(1)
t_tf = team_box.text_frame
t_tf.word_wrap = True
t_tf.margin_left = Inches(0.2)
t_tf.margin_top = Inches(0.1)

p = t_tf.paragraphs[0]
p.text = "TEAM ALERTSPEC ROSTER (ALL 6 MEMBERS)"
p.font.bold = True
p.font.size = Pt(10.5)
...
members = [
    ...
    ("2. Prerana Mondal", "Member", "Frontend Development, UI/UX, Data Visualization", DARK_NAVY),
    ("3. Sondeep Kumar", "Member", "Backend Development, Database, API Integration", DARK_NAVY),
    ("4. Shinjini Lohar", "Member", "AI/ML, Computer Vision, Data Processing", DARK_NAVY),
    ("5. Subham Kumar Modi", "Member", "GIS Analysis, Mobile Application, QA Testing", DARK_NAVY),
    ("6. Rahul Deo", "Member", "Cloud Infrastructure, DevOps, Testing & Security", DARK_NAVY)
]
...
# Footer for Slide 1
fl = slide1.shapes.add_textbox(Inches(0.8), Inches(7.05), Inches(9.5), Inches(0.3))
tf_fl = fl.text_frame
p_fl = tf_fl.paragraphs[0]
p_fl.text = "Team AlertNex | Smart India Hackathon 2026 | PS: SIH26001 | Disaster Management"
p_fl.font.size = Pt(9.5)
...
fr = slide1.shapes.add_textbox(Inches(10.5), Inches(7.05), Inches(2.033), Inches(0.3))
tf_fr = fr.text_frame
p_fr = tf_fr.paragraphs[0]
...
slide2.shapes.add_picture("sih_content_bg.png", 0, 0, SLIDE_W, SLIDE_H)
add_header_footer(slide2, 2, "THE CHALLENGE: Critical Vulnerabilities in North Eastern Region")

# Problem Summary Banner
sum_b = slide2.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(1.35), Inches(11.733), Inches(0.55))
sum_b.fill.solid()
sum_b.fill.fore_color.rgb = RGBColor(0xFE, 0xF2, 0xF2)
sum_b.line.color.rgb = RISK_RED
sum_b.line.width = Pt(1.2)
s_tf = sum_b.text_frame
s_tf.vertical_anchor = MSO_ANCHOR.MIDDLE
s_tf.margin_left = Inches(0.2)
p = s_tf.paragraphs[0]
...
ch_cards = [
    ...
    ("Zero Connectivity & Lag", "Severe storms knock out cell towers; authorities receive ground reports hours or days too late.", DARK_NAVY)
]

c_w = Inches(2.78)
c_h = Inches(2.25)
for i, (title, desc, col) in enumerate(ch_cards):
    left = Inches(0.8 + i * 2.98)
    card = slide2.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left, Inches(2.05), c_w, c_h)
    card.fill.solid()
...
comp_c = slide2.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(4.5), Inches(11.733), Inches(2.35))
comp_c.fill.solid()
comp_c.fill.fore_color.rgb = CARD_BG
comp_c.line.color.rgb = CARD_BORDER
comp_c.line.width = Pt(1.2)
c_tf = comp_c.text_frame
c_tf.word_wrap = True
...
curr_b = slide2.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(1.1), Inches(4.95), Inches(5.3), Inches(1.7))
curr_b.fill.solid()
...
# Our Approach Box
our_b = slide2.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(6.8), Inches(4.95), Inches(5.4), Inches(1.7))
our_b.fill.solid()
...
slide3.shapes.add_picture("sih_content_bg.png", 0, 0, SLIDE_W, SLIDE_H)
add_header_footer(slide3, 3, "OUR PROPOSED SOLUTION: Intelligent Disaster Monitoring Ecosystem")

# Left Column: Visual Ecosystem Diagram
eco_card = slide3.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(1.35), Inches(7.4), Inches(5.5))
eco_card.fill.solid()
eco_card.fill.fore_color.rgb = CARD_BG
...
risk_card = slide3.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(8.5), Inches(1.35), Inches(4.033), Inches(5.5))
...
risks = [
    ("GREEN: LOW RISK (0 - 30%)", "Normal terrain stability. Routine satellite and weather polling. Regular traffic on mountain highways.", RISK_GREEN),
    ("YELLOW: MODERATE RISK (31 - 60%)", "Elevated moisture saturation or continuous rainfall. Advisory issued to highway patrols; field officers alerted to monitor slopes.", RISK_YELLOW),
    ("ORANGE: HIGH RISK (61 - 80%)", "Heavy precipitation + steep slope trigger. Pre-warning alerts to transport authorities; heavy freight restricted; alternative routes prepared.", RISK_ORANGE),
    ("RED: CRITICAL RISK (81 - 100%)", "Imminent landslide danger. Instant Siren/SMS push to citizens, bypass emergency corridors activated, SDRF/NDRF teams pre-deployed.", RISK_RED)
]

for title, desc, col in risks:
...
slide4.shapes.add_picture("sih_content_bg.png", 0, 0, SLIDE_W, SLIDE_H)
add_header_footer(slide4, 4, "KEY INNOVATIONS: Why Team AlertNex Solution is Different")
...
innovations = [
    ("CARD 1: MULTI-SOURCE AI RISK ANALYSIS",
     ...
     ], DARK_NAVY, Inches(0.8), Inches(1.35)),
    ...
    ("CARD 2: CONNECTIVITY IMPACT ANALYSIS",
     ...
     ], SIH_ORANGE, Inches(6.8), Inches(1.35)),
...
slide5.shapes.add_picture("sih_content_bg.png", 0, 0, SLIDE_W, SLIDE_H)
add_header_footer(slide5, 5, "SYSTEM ARCHITECTURE: 4-Tier Flow & Technology Stack")
...
for i, (l_title, l_desc, l_col) in enumerate(arch_layers):
    ...
    layer_b.fill.solid()
    layer_b.fill.fore_color.rgb = CARD_BG
...
p_stack.font.color.rgb = WHITE

# ============================================================
# SLIDE 6: IMPACT, FEASIBILITY & FUTURE
# ============================================================
slide6 = prs.slides.add_slide(blank_layout)
slide6.shapes.add_picture("sih_content_bg.png", 0, 0, SLIDE_W, SLIDE_H)
add_header_footer(slide6, 6, "IMPACT, FEASIBILITY & FUTURE: The AlertNex Roadmap")
...
for title, points, col, left in columns:
    card = slide6.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left, Inches(1.35), c_w, c_h)
    card.fill.solid()
    card.fill.fore_color.rgb = CARD_BG
    card.line.color.rgb = col
    card.line.width = Pt(1.5)
...
# Save presentation
output_path = "SIH2026_AlertNex_SIH_Template_6Slides.pptx"
prs.save(output_path)
print(f"Presentation saved successfully to: {output_path}")

# Try also saving to original name if not locked by PowerPoint
try:
    prs.save("SIH2026_AlertNex_Official_6Slides.pptx")
    print("Also updated: SIH2026_AlertNex_Official_6Slides.pptx")
except Exception as e:
    print("SIH2026_AlertNex_Official_6Slides.pptx is currently open in PowerPoint, saved to SIH2026_AlertNex_SIH_Template_6Slides.pptx")
